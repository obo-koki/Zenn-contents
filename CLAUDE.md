...
- **CEO(人間)**: `org/concepts/` にコンセプトを追加する(または追加を依頼する)。
  ...
  発生する意思決定(有料API、外部サービス契約など)のみ。コンセプトの追加は、Claude Codeとの
  対話セッションのほか、スマホのLinearで `[Zenn]` から始まるIssueを自分にアサインする
  ことでも行える(`~/linear-claude-workspace/CLAUDE.md` の例外ルール経由)。
